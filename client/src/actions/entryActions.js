...
export const createEntry = signIn => async dispatch => {
	try {
		const res = await axios.post('http://localhost:5000/api/entries/add', { signIn }, AuthHeaders)
...
export const updateEntry = (id, payload) => async dispatch => {
	try {
		...
		const res = await axios.put(`http://localhost:5000/api/entries/${id}`, entry, AuthHeaders)
...
export const deleteEntry = (id, payload) => async dispatch => {
	try {
		const res = await axios.delete(`http://localhost:5000/api/entries/${id}`, AuthHeaders)

...
export const fetchEntries = () => async dispatch => {
	const res = await axios.get('http://localhost:5000/api/entries', AuthHeaders)
	dispatch({ type: FETCH_ENTRIES, payload: res.data })
}

export const createEntry = signIn => async dispatch => {
	try {
		const res = await axios.post(`${process.env.API_URL || 'http://localhost:5000'}/api/entries/add`, { signIn }, AuthHeaders)
...
export const updateEntry = (id, payload) => async dispatch => {
	try {
		...
		const res = await axios.put(`${process.env.API_URL || 'http://localhost:5000'}/api/entries/${id}`, entry, AuthHeaders)
...
export const deleteEntry = (id, payload) => async dispatch => {
	try {
		const res = await axios.delete(`${process.env.API_URL || 'http://localhost:5000'}/api/entries/${id}`, AuthHeaders)
